...
const dummyHeaderListFrom: HeaderItem[] = [{ id: 0, text: 'Token' }, { id: 1, text: 'Item Name' }, { id: 2, text: 'Variation Name' }, { id: 3, text: 'Unit and Precision' }];
const dummyHeaderListTo: HeaderItem[] = [{ id: 0, text: 'Title' }, { id: 1, text: 'URL handle' }, { id: 2, text: 'Description' }, { id: 3, text: 'Vendor' }, { id: 4, text: 'Product category' }];

function App() {
    // TODO: Reset initial value to '[]' when done testing
    const [headersFrom, setHeadersFrom] = useState<HeaderItem[]>(dummyHeaderListFrom);
    ...
    const parseFile = (event: ChangeEvent<HTMLInputElement>, type: 'from' | 'to'): void => {
    ...
    const headerClicked = (id: number, type: 'from' | 'to'): void => {
        console.log('Header clicked:', id);
    }

    return (
        <HeadersContext.Provider value={{headersFrom, headersTo, headerClicked}}>
            <MappingsContext.Provider value={{mappings, confirmMapping, deleteMapping}}>
                <div className="App">
                    ...
                    <div className="App-body">
                        <div className="App-body-left">
                            <FileLoader title="Load Columns From File" onFileLoad={(event: ChangeEvent<HTMLInputElement>) => { parseFile(event, 'from') }} />
                            <hr />
                            <HeaderList title="Columns Mapping From:" headers={headersFrom} type='from'/>
                        </div>
                        ...
                        <div className="App-body-right">
                            <FileLoader title="Load Columns To File" onFileLoad={(event: ChangeEvent<HTMLInputElement>) => { parseFile(event, 'to') }} />
                            <hr />
                            <HeaderList title="Columns Mapping To:" headers={headersTo} type='to'/>
                        </div>
                    </div>
